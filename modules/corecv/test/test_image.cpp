...
TEST(Image,TestSetImageToEmptyImage) {
    improc::Image image {};
    image.set_data(cv::UMat::zeros(10,10,CV_8U));
    EXPECT_FALSE(image.get_data().empty());
}

TEST(Image,TestSetInvalidImage) {
    improc::Image image {};
    EXPECT_THROW(image.set_data(cv::UMat::zeros(10,10,CV_16S)),improc::not_supported_data_type);
}

TEST(Image,TestImageConstructor) {
    improc::Image image {cv::UMat::zeros(10,10,CV_8U)};
    EXPECT_FALSE(image.get_data().empty());
}

TEST(Image,TestInvalidImageConstructor) {
    EXPECT_THROW(improc::Image image {cv::UMat::zeros(10,10,CV_16S)},improc::not_supported_data_type);
}

TEST(Image,TestGetImage) {
    cv::UMat image_data = cv::UMat::ones(3,5,CV_8U);
    improc::Image image {image_data};
    EXPECT_EQ(image.get_data(),image_data);
...
TEST(Image,TestCloneImage) {
    ...
    EXPECT_EQ(clone.get_data(),image_data);
    image_data.at<uint8_t>(0,0) = 20;
    EXPECT_EQ(image.get_data().at<uint8_t>(0,0),20);
